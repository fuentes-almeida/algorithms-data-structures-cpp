...
liga createNode(int item, char cadena[], int length, liga next) //Crea un nodo
{
    ...
    ptr->palabra=RequestVectorCharMem(length+1);                  //pide memoria para el puntero palabra, de tamano cadena
...
{
    if (pos>CalcularNumNodos(raiz)){
        printf("No existe ese nodo");
        return raiz;}
    liga ptr=raiz;
    int i=pos;
...
int CalcularTamLista(liga raiz)                     //calcula el tamano en total de todas las palabras
{
    ...
    return count;                                 //resta 1 por el ultimo espacio agregado
}

int CalcularNumNodos(liga raiz)                     //regresa el numero de nodos
{
