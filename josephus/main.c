...
{
    int j,n=atoi(argv[1]), k=atoi(argv[2]);     //los argumentos tipo char* recibidos se convierten a entero
    ...
    while (x!=x->next){                         //mientras no quede un solo nodo apuntando a si mismo
        ...
        free(t);                                //se libera el nodo (muere)
    }

    printf("\nSobrevive: %d \n",x->item);          //cuando ya solo queda un nodo apuntando a si mismo, se imprime el valor de ese nodo (sobrevive)
    return 0;
}
